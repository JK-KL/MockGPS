...
public class MainActivity extends Activity {
    ...
    public class Mytask extends AsyncTask<String, Integer, String> {
        ...
        public void getPoint() {
            ...
            Coodinate[] po = null;
            List<Coodinate> points = CaculatePoints(originalPoints);
//            String coodinate = "[\n" +
//                    "    {\n" +
//                    "        \"latitude\": 31.223928,\n" +
//                    "        \"longitude\": 121.64769\n" +
...
//            }
        ...
        public List<Coodinate> CaculatePoints(List<Coodinate> points) {
            List<Coodinate> po = new ArrayList<Coodinate>();
            GPSCaculation GPSC = new GPSCaculation();
            for (int i = 0; i < points.size() - 2; i++) {
                if ((points.get(i + 1).longitude - points.get(i).longitude == 0) || (points.get(i + 1).latitude
                        - points.get(i).latitude == 0)) {
                    po.add(points.get(i));
                    continue;
                }
                ...
                if (points.get(i + 1).longitude - points.get(i).longitude > 0) {
                    ...
                    for (int j = 0; j < n; j++) {
                        ...
                        BigDecimal y = GPSC.returnX(x.add(new BigDecimal(points.get(i).longitude)));
                        po.add(new Coodinate().setCood(y.doubleValue(), x.add(new BigDecimal(points.get(i).longitude)).doubleValue()));
                    }
                } else if (points.get(i + 1).latitude - points.get(i).latitude > 0) {
                    BigDecimal bd = new BigDecimal(points.get(i + 1).latitude).subtract(new BigDecimal(points.get(i).latitude)).setScale(6, RoundingMode.HALF_UP).multiply(new BigDecimal(1000000));
                    int n = (bd.intValue() > 100 ? 100 : bd.intValue());
                    for (int j = 0; j < n; j++) {
                        BigDecimal x = new BigDecimal(j).divide(new BigDecimal(1000000));
                        BigDecimal y = GPSC.returnY(x.add(new BigDecimal(points.get(i).latitude)));
                        po.add(new Coodinate().setCood(x.add(new BigDecimal(points.get(i).latitude)).doubleValue(), y.doubleValue()));
                    }
                }
            }
            return po;
